  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-6 mt-16 md:mt-0">
        {selectedTab === "stores" && (
          <button onClick={() => setShowCreateForm(!showCreateForm)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 mb-4  ">
            <FaPlus /> Create Store
          </button>
        )}

        {showCreateForm && selectedTab === "stores" && (
          <form onSubmit={handleCreateStore} className="bg-white p-4 shadow-md ">
            <h2 className="text-lg font-bold mb-2">Create New Store</h2>
            <input type="text" placeholder="Store Name" value={newStore.name} onChange={(e) => setNewStore({ ...newStore, name: e.target.value })} className="w-full p-2 mb-2 border border-gray-300  " />
            <input type="email" placeholder="Store Email" value={newStore.email} onChange={(e) => setNewStore({ ...newStore, email: e.target.value })} className="w-full p-2 mb-2 border border-gray-300  " />
            <input type="text" placeholder="Store Image URL" value={newStore.image} onChange={(e) => setNewStore({ ...newStore, image: e.target.value })} className="w-full p-2 mb-2 border border-gray-300  " />
            <input type="text" placeholder="Store Address" value={newStore.address} onChange={(e) => setNewStore({ ...newStore, address: e.target.value })} className="w-full p-2 mb-2 border border-gray-300  " />
            <input type="text" placeholder="Owner ID" value={newStore.owner_id} onChange={(e) => setNewStore({ ...newStore, owner_id: e.target.value })} className="w-full p-2 mb-2 border border-gray-300  " />
            <button type="submit" className="w-full bg-black text-white py-2 ">Add Store</button>
          </form>
        )}

        {selectedTab === "stores" ? <StoreList /> : <UserList />}
      </main>
    </div>
  );
};
